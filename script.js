...
/* Xash3D port data mirrored from xashstation.github.io */
const XASH_PORTS = [
    ...
    try {
      ...
      return `
        <div class="dev-card glass reveal reveal-delay-${idx + 1}">
          ...
          ${user.bio ? `<div class="dev-bio"><p>${user.bio}</p></div>` : ''}
          <div class="dev-stats">
            <div class="dev-stat">
          ...
          ${topLangs.length ? `
          <div class="dev-tags">
            ${topLangs.map(l => `<span class="dev-tag" style="border-color:${getLangColor(l)}40; color:${getLangColor(l)}">${l}</span>`).join('')}
          </div>` : ''}
          <div class="dev-links">
